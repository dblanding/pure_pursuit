...
def update(frame, ax_polar, ax_linear, client_ref):
    ...
    ax_polar.set_title("Polar Histogram", color="white", pad=10)
    ...
    ax_polar.plot(theta_ring,
                  ...
                  "w--", linewidth=1, alpha=0.4, label=f"threshold={OBSTACLE_THRESHOLD}")

    # Goal direction
    if pose and goal:
        # Convert robot-frame bearing back to polar-plot angle
        # (plot is CW from N = robot forward)
        goal_plot_angle = -preferred  # negate for CW
        ax_polar.annotate("", xy=(goal_plot_angle, OBSTACLE_THRESHOLD * 2.5),
                          xytext=(0, 0),
                          arrowprops=dict(arrowstyle="->", color="#ff9900",
                                          lw=2))
        ax_polar.text(goal_plot_angle, OBSTACLE_THRESHOLD * 2.8, "goal",
                      color="#ff9900", ha="center", fontsize=8)

    # Chosen valley direction
    if chosen:
        chosen_plot_angle = -chosen["angle"]
        ax_polar.annotate("", xy=(chosen_plot_angle, OBSTACLE_THRESHOLD * 2.5),
                          xytext=(0, 0),
    ...
    ax_polar.set_ylim(0, OBSTACLE_THRESHOLD * 3.5)
